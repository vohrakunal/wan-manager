...
const META = {
  nginx:          { label: 'Nginx',          desc: 'Web / reverse-proxy — cloud.panicle.in + port forwarding' },
  mongod:         { label: 'MongoDB',        desc: 'Database server — ports 27017-27018' },
  'isc-dhcp-server': { label: 'DHCP Server', desc: 'ISC DHCP lease manager — pool 192.168.1.100-200 on eno1' },
  openvpn:        { label: 'OpenVPN',        desc: 'VPN tunnel daemon' },
  iperf3:         { label: 'iPerf3',         desc: 'Network throughput test server — port 5201 (LAN only)' },
  docker:         { label: 'Docker',         desc: 'Container runtime' },
  fail2ban:       { label: 'Fail2Ban',       desc: 'SSH brute-force protection' },
  lldpd:          { label: 'LLDPD',          desc: 'Link Layer Discovery Protocol — neighbour detection' },
  nmbd:           { label: 'Samba NMB',      desc: 'NetBIOS name service — bound to eno1 + lo' },
  smbd:           { label: 'Samba SMB',      desc: 'File sharing (SMB/CIFS) — LAN only, ports 139/445' },
  ...
  aria2:          { label: 'aria2',          desc: 'Download manager — RPC port 6800, saves to NAS/downloads' },
